 ...
 *
 * Ne demande aucune information médicale : il sert uniquement à être recontacté.
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4 rounded-3xl border border-brand-200 bg-brand-50 p-10 text-center">
        ...
        <p className="max-w-sm text-sm leading-relaxed text-zinc-600">
          Merci pour votre message. Le secrétariat vous répondra dans les plus
          brefs délais.
        </p>
        <button
          type="button"
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          required
          placeholder="Votre message (sans information médicale détaillée)…"
          className={inputClass}
        />
      </div>
      ...
      {status === "error" ? (
        <p className="flex items-center gap-2 text-sm text-red-600">
          ...
          Une erreur est survenue. Merci de réessayer ou d&apos;appeler le
          cabinet.
        </p>
      ) : null}
      ...
      <p className="flex items-start gap-2 rounded-xl bg-zinc-50 px-4 py-3 text-xs leading-relaxed text-zinc-500">
        <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" aria-hidden="true" />
        Ce formulaire ne remplace pas les services d&apos;urgence et ne doit pas
        être utilisé pour transmettre des informations médicales sensibles.
      </p>
    </form>
  );
}