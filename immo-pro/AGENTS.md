...
- Home is assembled in `app/page.tsx` from sections in `components/` (`Hero`, `PropertyGrid`, `FeaturedProperty`, `About`, `Services`, `ValuationCTA`, `Testimonials`, `Location`, `CtaFinal`). Contact is a dedicated page: `app/contact/page.tsx` renders a banner + `components/Contact.tsx` (info + `ContactForm`); nav/footer/CTA links point to `/contact` (not an anchor). `Header`/`Footer`/`WhatsAppButton` and the `RealEstateAgent` JSON-LD live in `app/layout.tsx`; each property page adds an `Offer` JSON-LD. `app/sitemap.ts` also lists `/contact`.
